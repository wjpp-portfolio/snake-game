...
class Button:
    ...
    def is_highlighted(self) -> bool:
        """when mouse is over or this button is the active item withouth yet being clicked"""
        self.highlighted = False
...
class Menu:
    def __init__(self,order,first_button_loc: Optional[Coordinate] = None,button_spacing_y=50, button_spacing_x=100):
        ...
        """
        #if explicit, do not need a location set in class
            #but button must hve location
            #no need for spacing
        #if horizontal or vertical, class needs y,x of first button AND spacing
...
class Snake:
    ...
    def set_speed(self,speed: int) -> None:
        """sets snake speed up to FPS limit"""
        if speed > FPS:
    ...
    def move(self,seconds) -> None: 
        """updates the self.segments list location to move the snake"""
    ...
    def collided_with_opponent(self, list_of_snakes: list) -> bool:
        """dtermines if snake has crashed into other snake"""
    ...
    def collided_with_self(self) -> bool:
        ...
        for segs in self.segments[1:]:
            if head.location == segs.location:
                return True
class Game_Logic:
    """used to handle specific elements of gameplay such as creating food"""
    ...
    def check_food_collision(self) -> None:
        """determines whether either snake is ont he same location as food"""
    ...
    def check_collisions(self) -> bool:
        """returns check snake collision with edge of map, self, and other snake"""
...
class Game_display:
    """draws snakes and food on screen"""
    def __init__(self):
    ...
    def show_score(self,winner,score: int) -> None: 
        """shows score (number of segments) and counts down to next game  """
    
        for i in range(3, 0, -1):
            self.display.fill(WHITE)
            score_text = self.font.render(f'{winner} wins! Score: {score}', True, BLACK)
            score_rect = score_text.get_rect(center=(GRID_SIZE[1]/2, GRID_SIZE[0]/3))
            self.display.blit(score_text, score_rect)
 
            countdown_text = self.font.render(str(i), True, BLACK)
...
def session_two_player(screen: Game_display, players: int) -> int:
    ...
    while True: 
        ...
        screen.show(game) #updates visuals
...
def main():
    ...
    try:
        ...
        while True:
            winner = session_two_player(screen,player_selection)
